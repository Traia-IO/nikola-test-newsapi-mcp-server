#!/usr/bin/env python
"""
Nikola TEST - NewsAPI MCP Server Health Check Script

This script properly connects to the Nikola TEST - NewsAPI MCP server and checks its health by:
...
def check_mcp_server_health(url: str) -> bool:
    """Check if Nikola TEST - NewsAPI MCP server is healthy"""
    print(f"🔍 Checking Nikola TEST - NewsAPI MCP server health at {url}")
    ...
    if "error" in result and "session" not in str(result.get("error", "")):
        ...
        if "result" in result and "tools" in result["result"]:
            ...
            if missing_tools:
                ...
                print("✅ All expected Nikola TEST - NewsAPI tools are available!")
...
def main():
    parser = argparse.ArgumentParser(description="Check Nikola TEST - NewsAPI MCP Server Health")
    parser.add_argument("--url", default="http://localhost:8000", 
                       help="Nikola TEST - NewsAPI MCP server URL (default: http://localhost:8000)")
    args = parser.parse_args()
    
    print(f"🚀 Nikola TEST - NewsAPI MCP Server Health Check")
    print(f"📍 Server URL: {args.url}")
    print(f"📰 Expected tools: example_tool, get_api_info")
    print("="*50)
    
    if check_mcp_server_health(args.url):
        print("\n✅ Nikola TEST - NewsAPI MCP Server is healthy and responding!")
        return 0
    else:
        print("\n❌ Nikola TEST - NewsAPI MCP Server health check failed!")
        return 1

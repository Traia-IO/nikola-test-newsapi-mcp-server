...
# Script to build and run the Nikola TEST - NewsAPI MCP Server locally in Docker
...
IMAGE_NAME="nikola-test-newsapi-mcp-server"
CONTAINER_NAME="nikola-test-newsapi-mcp-local"
HOST_PORT=${PORT:-8000}
CONTAINER_PORT=${PORT:-8000}

echo -e "${BLUE}🚀 Building and running Nikola TEST - NewsAPI MCP Server...${NC}"
echo
...
if [ ! -f .env ]; then
    ...
    if [ -f .env.example ]; then
        ...
        cat > .env << EOF
# Nikola TEST - NewsAPI MCP Server Configuration
PORT=8000
STAGE=MAINNET
LOG_LEVEL=INFO
...
echo
echo -e "${GREEN}✅ Nikola TEST - NewsAPI MCP Server is running!${NC}"
echo
